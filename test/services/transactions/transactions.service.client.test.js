...
if (!env.includes(process.env.NODE_ENV) || process.argv.includes('--noclient')) {
  ...
  console.log('SKIPPED - Test transactions/transactions.service.client.test.js');
...
describe('Test transactions/transactions.service.client.test.js', () => {
  let app;
  let client;
  let server;
  ...
  before(async function () {
    ...
    const result = await app.service('/transactions').find({ query: { email }});
    if ((result.data || result).length === 0) {
      await app.service('/transactions').create({ email, password });
    }
  ...
  beforeEach(async () => {
    await app.service('/transactions').remove(null);
  });
  ...
  it('registered the service', () => {
    const service = client.service('/transactions');
  ...
  it('???', async () => {
    ...
    /*
    const record = await client.service('/transactions').create({
